...
#' then the function uses the order in \code{cvFit}.
#'
#' @return An object of class \code{table}.
#'
#' @export
calcConfusionCv = function(cvFit, lambda, ematMerged, sampleMetadata, className='class', classLevels=NA) {
	...
	trueClass = factor(sampleMetadata[colnames(ematMerged), className], levels=classLevels)
...
#' @param classLevels Order of classes in the confusion matrix. If \code{NA} (default),
#' then the function uses the order in \code{cvFit}.
#' @param each logical indicating whether to calculate a confusion matrix for each validation
#' dataset (default) or one confusion matrix including all datasets.
#'
#' @return If \code{each==TRUE}, a list of objects of class \code{table}. Otherwise,
#' an object of class \code{table}.
#'
#' @export
calcConfusionValidation = function(predsList, lambda, sampleMetadata, className='class', classLevels=NA, each=TRUE) {
	if (is.na(classLevels[1])) {
	...
	if (each) {
		...
		for (validationStudyName in names(predsList)) {
			...
			trueClass = factor(sampleMetadata[rownames(predsProb), className], levels=classLevels)
